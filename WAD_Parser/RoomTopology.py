...
def _plot_room(rooms, node):
    ...
    for l in node['walls']:
        ...
        if l[1] == 0:
            ...
            color = 127
        rr, cc = line(int(np.floor(seg[0][0])), int(np.floor(seg[0][1])), int(np.floor(seg[1][0])),
                      int(np.floor(seg[1][1])))
        img[rr, cc] = color
    imshow(img)
    ...
    try:
        ...
        if room_coordinates:
            ...
            for floor_id in range(max(1, floors.min()), floors.max() + 1):  # Skipping label 0 (background)
                ...
                floor_contour = \
                    find_contours(resize(floors == floor_id, (floors.shape[0] * 2, floors.shape[1] * 2), order=0), 0.5,
                ...
                for room_id in range(max(1, rooms.min()), rooms.max() + 1):  # Skipping label 0 (background)
                    ...
                    # This is for avoiding "X" shaped connections between rooms due to how find_contours work
                    room_contour = \
                        find_contours(resize(rooms == room_id, (rooms.shape[0] * 2, rooms.shape[1] * 2), order=0), 0.5,
                                      fully_connected='high', positive_orientation='low')[0] / 2
                ...
                for room_id in range(max(1, rooms.min()), rooms.max() + 1):
                    if room_id not in rooms:
                        # Some room id may be in another floor, if they are enumerated horizontally
                        continue
                    boundaries_current = {wall for wall in room_RAG_boundaries.node[room_id]['walls'] if
                                          wall[1] is None}
                    for neigh in room_RAG_boundaries.adj[room_id]:
                        ...
                        boundaries_neigh = {wall for wall in room_RAG_boundaries.node[neigh]['walls'] if
                                            wall[1] is None}
                        boundaries_neigh_reverse = {_reverse_wall(wall) for wall in
...
def topological_features(floormap, prepare_for_doom=False):
    ...
    for i, j in room_graph.edges():
        ...
        centroid_distance[(i, j)] = np.linalg.norm(
            np.asarray(room_graph.nodes[i]["centroid"]) - np.asarray(room_graph.nodes[j]["centroid"])).item()
    ...
    diam_list = list()
    assort_list = list()
    for cc in connected_component_subgraphs(graph_no_background):
        if len(cc.edges()) > 0:
        ...
        metrics["assortativity-mean"] = nx.degree_assortativity_coefficient(graph_no_background) if len(
            cc.edges()) > 0 else 0
    try:
        # Centrality measures
